...
;;; Commentary:
;; 基于 tabulated-list-mode 的 Claude Code 实例管理面板。
;; 支持查看所有运行中的实例、当前活动状态、创建/关闭实例、
;; 批量删除、切换实例时同步 input 窗口。
...
(declare-function claude-code-input-mode "init-claude")
     ...
     (lambda (buf)
       (let* ((buf-name (buffer-name buf))
              ...
              (instance (or (claude-code--extract-instance-name-from-buffer-name buf-name)
                            "default"))
              (character (or (claude-code--get-character-id buf) ""))
         ...
         (list buf-name
               (vector mark instance character dir status activity))))
     buffers)))
